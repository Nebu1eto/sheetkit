 ...
 *
 * Usage:
 *   node --expose-gc benchmarks/node/bench-baseline.mjs
 */
...
function fixturePath(name) {
  return fileURLToPath(fixtureUrl(name));
}
...
function fileSizeKb(name) {
  try {
    return statSync(fixtureUrl(name)).size / 1024;
  } catch {
    return null;
  }
}
...
async function main() {
  ...
  console.log(`Config: ${WARMUP_RUNS} warmup + ${BENCH_RUNS} measured runs per scenario`);
  if (!globalThis.gc) {
    console.log('WARNING: --expose-gc not enabled. RSS measurements may be less accurate.');
  }
  console.log('');

  const available = FIXTURES.filter((f) => {
    if (!fixtureExists(f.name)) {
      console.log(`SKIP: ${f.name} not found. Run 'pnpm generate' in benchmarks/node first.`);
      return false;
    }
    return true;
  });
  ...
  for (const f of available) {
    ...
    const sizeStr = sizeKb != null ? `${(sizeKb / 1024).toFixed(1)}MB` : 'unknown size';
  ...
  for (const f of available) {
    ...
    allResults.push({ ...result, category: 'open-path-async', fixture: f.label });
  }
  console.log('');
  ...
  for (const f of available) {
    ...
    allResults.push({ ...result, category: 'openBufferSync', fixture: f.label });
  }
  console.log('');
  ...
  for (const f of available) {
    ...
    allResults.push({ ...result, category: 'openBuffer-async', fixture: f.label });
  }
  console.log('');
  ...
  // 7. getRows on pre-opened workbook (isolate getRows cost from open cost)
  ...
  for (const f of available) {
    ...
    allResults.push({ ...result, category: 'getRows-only', fixture: f.label });
  }
  console.log('');
  ...
  for (const f of available) {
    ...
    allResults.push({ ...result, category: 'getRowsRaw-only', fixture: f.label });
  }
  console.log('');
  ...
  // Write JSON baseline
  const outputPath = fileURLToPath(new URL('./baseline-results.json', import.meta.url));
  const jsonOutput = {
