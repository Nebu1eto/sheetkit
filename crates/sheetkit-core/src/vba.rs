...
pub fn extract_vba_modules(vba_bin: &[u8]) -> Result<Vec<VbaModule>> {
    ...
    // Parse module entries from the decompressed dir stream.
    let entries = parse_dir_stream(&decompressed_dir)?;
    ...
    for entry in entries {
        ...
        // The module stream has a text_offset bytes of "performance cache"
        ...
        // VBA source is typically encoded in a codepage (often Windows-1252),
        // but we do best-effort UTF-8. Invalid bytes are replaced.
...
/// Parse the decompressed `dir` stream to extract module entries.
///
/// The dir stream is a sequence of records with 2-byte IDs and 4-byte sizes.
/// We look for MODULE_NAME, MODULE_STREAM_NAME, MODULE_OFFSET, and
/// MODULE_TYPE records.
fn parse_dir_stream(data: &[u8]) -> Result<Vec<ModuleEntry>> {
    let mut pos = 0;
    ...
    while pos + 6 <= data.len() {
        ...
        match record_id {
            ...
            // MODULETYPE procedural
            0x0021 => {
                current_type = VbaModuleType::Standard;
            }
            // MODULETYPE class/document
            0x0022 => {
                // Could be Class, Document, Form, or ThisWorkbook.
                // We infer from the name.
    ...
    // Refine module types based on name heuristics
    for module in &mut modules {
        if module.module_type != VbaModuleType::Standard {
            let name_lower = module.name.to_lowercase();
            if name_lower == "thisworkbook" {
                module.module_type = VbaModuleType::ThisWorkbook;
            } else if name_lower.starts_with("sheet") {
                module.module_type = VbaModuleType::Document;
            } else if module.source_code_looks_like_form(&module.name) {
                module.module_type = VbaModuleType::Form;
            }
        }
    }
...
impl ModuleEntry {
    /// Heuristic: forms usually have "UserForm" in a naming convention.
    fn source_code_looks_like_form(&self, _name: &str) -> bool {
        false
    }
}
...
mod tests {
    ...
    fn test_get_vba_modules_from_test_project() {
        ...
        let wb = crate::workbook::Workbook::open_from_buffer(&buf).unwrap();
        let modules = wb.get_vba_modules().unwrap();
        assert!(modules.is_some(), "should have VBA modules");
        let modules = modules.unwrap();
        assert_eq!(modules.len(), 1);
        assert_eq!(modules[0].name, "Module1");
        assert_eq!(modules[0].module_type, VbaModuleType::Standard);
        assert!(
            modules[0].source_code.contains("Sub Hello()"),
    ...
    fn test_vba_project_preserved_in_save_roundtrip() {
        ...
        let modules = wb2.get_vba_modules().unwrap().unwrap();
        assert_eq!(modules.len(), 1);
        assert_eq!(modules[0].name, "Module1");
    }
