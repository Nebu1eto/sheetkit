...
- **sheetkit-node** (`packages/sheetkit`): Node.js bindings via napi-rs. The Rust crate produces a cdylib (`binding.js`/`binding.d.ts`). TypeScript source files (`index.ts`, `buffer-codec.ts`, `sheet-data.ts`) provide the public API wrapper. SWC transpiles to JS; tsc generates `.d.ts` declarations.
...
TypeScript source files (`index.ts`, `buffer-codec.ts`, `sheet-data.ts`) provide the public API. These import from `binding.js`/`binding.d.ts` (generated by napi-rs) and add a JS `Workbook` wrapper class that delegates to the native `Workbook`. SWC transpiles `.ts` to `.js`; tsc generates `.d.ts` declarations. The generated `.js` and `.d.ts` files are gitignored.

Build pipeline: `build:napi` (Rust compile + napi codegen) -> `typecheck` (tsc --noEmit) -> `build:js` (SWC) -> `build:types` (tsc declarations).
...
- `pnpm build` runs: napi build -> typecheck -> SWC transpile -> tsc declarations. Generated `.js`/`.d.ts` are gitignored.
