...
Node.js 바인딩을 통해 50,000행 x 20열 시트를 읽을 때, 기존 `getRows()` 구현은 1,000,000개 이상의 개별 `JsRowCell` napi 객체를 생성했다. 각 객체 생성은 Rust/JS FFI 경계를 넘고 V8 힙 메모리를 할당한다. 메모리에 남아 있는 Rust 측 워크북 데이터와 합치면, 실제 셀 데이터는 약 50MB에 불과한 시나리오에서 약 400MB의 메모리를 사용하게 된다.

근본 원인은 napi 객체 생성이 확장되지 않는다는 것이다: 오버헤드는 데이터 크기가 아닌 셀 수에 비례한다. 100만 셀 시트의 경우, napi 오버헤드만으로 200MB 이상의 V8 힙 할당이 발생하고, 셀 참조, 타입 문자열, 값 문자열에 대한 300만 건 이상의 개별 힙 할당으로 인해 Rust 측 시스템 할당자 오버헤드가 약 72MB 추가된다.

### 해결 방법

SheetKit은 Buffer 기반 FFI 전송 전략을 사용한다. 셀 하나마다 JS 객체를 생성하는 대신, Rust 측에서 시트의 전체 셀 데이터를 단일 압축 바이너리 `Buffer`로 직렬화한다. 이 Buffer는 한 번의 호출로 FFI 경계를 넘는다. JavaScript 측에서는 코덱 레이어가 Buffer를 호출자가 필요로 하는 형태로 디코딩한다.
...
결정 규칙은 단순하다: 페이로드가 셀 수에 비례하여 증가하면 Buffer 전송을 사용한다. 페이로드가 고정 크기이거나 작은 경우 (단일 셀 값, 스타일 설정, 시트 속성)에는 직접 napi 호출을 사용한다.

### 바이너리 Buffer 형식

모든 다중 바이트 값은 리틀 엔디안이다. Buffer는 네 개의 섹션으로 구성된다.
...
| 12 | 4 | flags | 비트 0: sparse 플래그. 비트 16-31: min_col 오프셋 |

**행 인덱스** (row_count * 8바이트):

각 행 항목은 8바이트이다: `u32` 행 번호와 `u32` 셀 데이터 섹션 내 바이트 오프셋. 빈 행은 센티넬 오프셋 `0xFFFFFFFF`를 사용한다.
...
| offsets | count * 4 | blob 내 각 문자열의 바이트 오프셋 |
| blob | blob_size | 연결된 UTF-8 문자열 데이터 |

문자열은 셀 페이로드에서 인덱스로 참조된다. JavaScript 측에서는 `TextDecoder`를 사용하여 Buffer에서 직접 UTF-8 슬라이스를 복사 없이 디코딩한다.

**셀 데이터**:

각 셀은 1바이트 타입 태그와 8바이트 페이로드로 인코딩된다. 타입 태그는 다음과 같다:
...
셀당 9바이트 고정 스트라이드는 dense 모드에서 O(1) 임의 접근을 가능하게 한다: (row, col) 위치의 셀 오프셋은 `cell_data_start + row_offset + col * 9`이다.

### Dense/Sparse 자동 전환

Buffer 인코더는 경계 사각형 내의 셀 밀도에 따라 dense와 sparse 레이아웃 사이에서 자동으로 선택한다.

**밀도 임계값**: 점유된 셀이 30%.

- **Dense 레이아웃** (밀도 >= 30%): 셀 데이터 섹션은 `row_count * col_count * 9` 바이트의 평면 그리드이다. 빈 셀을 포함하여 경계 사각형의 모든 위치에 9바이트 슬롯이 있다. 이를 통해 O(1) 직접 오프셋 주소 지정이 가능하다: `offset = row_index_offset + col * 9`.

- **Sparse 레이아웃** (밀도 < 30%): 각 행은 `u16` 셀 수와 가변 길이 항목을 저장한다. 각 항목은 11바이트이다: `u16` 열 인덱스 + `u8` 타입 + 8바이트 페이로드. 비어 있지 않은 셀만 저장한다. 이를 통해 데이터가 넓은 범위에 산발적으로 분포된 시트에서 공간 낭비를 방지한다.

sparse 플래그는 헤더의 flags 필드 비트 0에 인코딩된다. JavaScript 디코더는 이 플래그를 읽고 디코딩 전략을 전환한다.

### 내부 최적화

Rust XML 레이어의 세 가지 최적화가 Buffer 직렬화 이전의 파싱 단계에서 메모리 할당을 줄인다.

**CellTypeTag 열거형**: `<c>` 요소의 XML `t` 속성 (셀 타입을 나타냄: `s`는 공유 문자열, `n`은 숫자, `b`는 불리언 등)은 이전에 `Option<String>`으로 저장되었다. 이는 모든 셀마다 힙 `String`을 할당했다. 현재는 `None`, `SharedString`, `Number`, `Boolean`, `Error`, `Str`, `Date`, `InlineString` variant를 가진 1바이트 `CellTypeTag` 열거형이다. 100만 셀 기준 약 24MB의 힙 할당을 제거한다.

**CompactCellRef**: `<c>` 요소의 XML `r` 속성 (셀 참조, 예: "A1", "B2", "AA100")은 이전에 힙 할당 `String`이었다. 현재는 `[u8; 10]` 인라인 바이트 배열과 `u8` 길이 필드를 포함하는 `CompactCellRef` 구조체이다. 셀 참조는 10자를 초과하지 않으므로 (최대값은 "XFD1048576"로 10바이트), 전체가 스택이나 `Cell` 구조체 내부에 인라인으로 위치하여, 100만 셀당 약 28MB의 힙 할당을 제거한다.

**u32 열 번호**: `get_rows()` 내부 경로는 이전에 Rust 레이어 내에서 열 번호를 열 이름 문자열로 변환했다 (예: 1을 "A"로, 27을 "AA"로). 셀마다 `String`을 할당했다. 현재 열 번호는 내부적으로 `u32` 값으로 반환되며, 이름 변환은 필요한 경우에만 JavaScript 디코더에서 수행된다. 이를 통해 100만 셀당 약 5MB의 문자열 할당을 제거한다.
