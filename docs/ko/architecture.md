...
StreamWriter는 대용량 .xlsx 파일을 생성하기 위한 전방 전용 API를 제공합니다. 전체 워크시트 XML을 메모리에 구축하는 대신, 행이 추가될 때 XML 요소를 출력에 직접 기록합니다. 제약 사항:

- 행은 오름차순으로 작성해야 합니다 (임의 접근 불가).
- `flush()`가 호출되면 시트 XML이 완료됩니다.
- 스트림의 SST 항목은 flush 시 워크북 SST에 병합됩니다.
