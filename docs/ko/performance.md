...
|---------|----------|---------|---------|
| 대용량 읽기 (50k 행 x 20 열) | 546ms | 2.91s | 1.65s |
| 대용량 쓰기 (50k 행 x 20 열) | 489ms | 2.73s | 1.37s |
| 버퍼 왕복 (10k 행) | 131ms | 498ms | 198ms |
| 랜덤 접근 읽기 (50k 행 파일에서 1k 셀) | 489ms | 3.04s | 1.37s |
...
|---------|----------|----------|-----------------|-----------|
| 대용량 읽기 (50k 행) | 525ms | 331ms | N/A | 39ms* |
| 대용량 쓰기 (50k 행 x 20 열) | 527ms | N/A | 940ms | 971ms |
| 스트리밍 쓰기 (50k 행) | 202ms | N/A | 931ms | N/A |
...
| 작업 | 오버헤드 |
|------|----------|
| **읽기 작업 (sync)** | 약 1.05배 (일반적으로 약 5% 느림) |
| **읽기 작업 (async)** | 약 1.02배 (일반적으로 약 2% 느림) |
...
|---------|------|---------|----------|
| 대용량 데이터 (50k 행 x 20 열) | 518ms | 546ms | +5% |
| 많은 스타일 (5k 행, 서식 적용) | 27ms | 29ms | +7% |
| 다중 시트 (10개 시트 x 5k 행) | 301ms | 625ms | +108% |
| 수식 (10k 행) | 33ms | 42ms | +27% |
| 문자열 (20k 행 텍스트 중심) | 108ms | 112ms | +4% |

### 쓰기 성능 비교

| 시나리오 | Rust | Node.js | 오버헤드 |
|---------|------|---------|----------|
| 50k 행 x 20 열 | 503ms | 489ms | -3% (더 빠름) |
| 5k 스타일 적용 행 | 28ms | 36ms | +29% |
| 10k 행 (수식 포함) | 24ms | 30ms | +25% |
| 20k 텍스트 중심 행 | 107ms | 90ms | -16% (더 빠름) |
...
| 1k | 5ms | 5ms | 0% |
| 10k | 47ms | 48ms | +2% |
| 50k | 247ms | 244ms | -1% (더 빠름) |
| 100k | 518ms | 508ms | -2% (더 빠름) |
