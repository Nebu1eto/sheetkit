...
|---------|----------|---------|---------|
| 대용량 읽기 (50k 행 x 20 열) | 454ms | 2.69s | 1.59s |
| 대용량 쓰기 (50k 행 x 20 열) | 461ms | 2.55s | 1.25s |
| 버퍼 왕복 (10k 행) | 118ms | 479ms | 147ms |
| 랜덤 접근 읽기 (50k 행 파일에서 1k 셀) | 387ms | 2.81s | 1.29s |
...
|---------|----------|----------|-----------------|-----------|
| 대용량 읽기 (50k 행) | 390ms | 299ms | N/A | 35ms |
| 대용량 쓰기 (50k 행 x 20 열) | 459ms | N/A | 847ms | 886ms |
| 스트리밍 쓰기 (50k 행) | 184ms | N/A | 858ms | N/A |
| 수정 (50k 행 파일에서 1k 셀) | 588ms | N/A | N/A | N/A |
...
| 작업 | 오버헤드 |
|------|----------|
| **읽기 작업 (sync)** | 약 1.20배 (일반적으로 약 20% 느림) |
| **읽기 작업 (async)** | 약 1.15배 (일반적으로 약 15% 느림) |
| **쓰기 작업 (batch)** | 약 1.0배 (거의 동일) |
| **스트리밍 쓰기** | 1.66배 (66% 느림) |
| **버퍼 왕복** | 1.11배 (11% 느림) |
...
|---------|------|---------|----------|
| 대용량 데이터 (50k 행 x 20 열) | 387ms | 454ms | +17% |
| 많은 스타일 (5k 행, 서식 적용) | 20ms | 24ms | +20% |
| 다중 시트 (10개 시트 x 5k 행) | 223ms | 530ms | +138% |
| 수식 (10k 행) | 24ms | 33ms | +38% |
| 문자열 (20k 행 텍스트 중심) | 83ms | 95ms | +14% |

### 쓰기 성능 비교

| 시나리오 | Rust | Node.js | 오버헤드 |
|---------|------|---------|----------|
| 50k 행 x 20 열 | 478ms | 461ms | -4% (더 빠름) |
| 5k 스타일 적용 행 | 25ms | 35ms | +40% |
| 10k 행 (수식 포함) | 21ms | 28ms | +33% |
| 20k 텍스트 중심 행 | 92ms | 87ms | -5% (더 빠름) |
