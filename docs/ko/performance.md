...
|---------|----------|----------|-----------------|-----------|
| 대용량 읽기 (50k 행) | 497ms | 322ms | N/A | 39ms* |
| 대용량 쓰기 (50k 행 x 20 열) | 488ms | N/A | 911ms | 937ms |
| 스트리밍 쓰기 (50k 행) | 195ms | N/A | 910ms | N/A |
| 수정 (50k 행 파일에서 1k 셀, lazy) | 671ms | N/A | N/A | N/A |
...
| **읽기 작업 (async)** | 약 1.05배 (일반적으로 약 5% 느림) |
| **쓰기 작업 (batch)** | 약 0.92배 (V8 문자열 처리로 더 빠름) |
| **스트리밍 쓰기** | 1.53배 (53% 느림) |
| **버퍼 왕복** | 약 1.0배 (거의 동일) |
...
|---------|------|---------|----------|
| 대용량 데이터 (50k 행 x 20 열) | 499ms | 541ms | +8% |
| 많은 스타일 (5k 행, 서식 적용) | 26ms | 27ms | +4% |
| 다중 시트 (10개 시트 x 5k 행) | 299ms | 290ms | -3% (더 빠름) |
| 수식 (10k 행) | 33ms | 34ms | +3% |
| 문자열 (20k 행 텍스트 중심) | 107ms | 117ms | +9% |

### 쓰기 성능 비교

| 시나리오 | Rust | Node.js | 오버헤드 |
|---------|------|---------|----------|
| 50k 행 x 20 열 | 510ms | 469ms | -8% (더 빠름) |
| 5k 스타일 적용 행 | 28ms | 32ms | +14% |
| 10k 행 (수식 포함) | 23ms | 27ms | +17% |
| 20k 텍스트 중심 행 | 105ms | 86ms | -18% (더 빠름) |
...
| 50k | 251ms | 231ms | -8% (더 빠름) |
| 100k | 515ms | 485ms | -6% (더 빠름) |
