...
StreamWriter는 행 수에 관계없이 일정한 메모리 사용량으로 대용량 시트를 작성하는 순방향 전용 스트리밍 API입니다. 각 `write_row()` 호출은 행을 XML로 serialize하여 디스크의 임시 파일에 직접 기록합니다. 저장 시 임시 파일에서 ZIP 아카이브로 행 데이터를 직접 스트리밍하여, 전체 시트 XML이 메모리에 존재하지 않습니다.

행은 반드시 오름차순으로 작성해야 합니다. 열 너비, 스타일, 틀 고정 등은 행을 쓰기 전에 설정해야 합니다. `apply_stream_writer()` 이후 스트리밍된 시트의 셀 값은 직접 읽을 수 없으며, 데이터를 읽으려면 저장 후 다시 열어야 합니다.
...
| `set_col_width_range` | 열 범위의 너비 설정                           |
| `set_freeze_panes`    | 틀 고정 설정                                  |
| `set_col_style`       | 열 스타일 설정                                |
| `set_col_visible`     | 열 가시성 설정                                |
| `set_col_outline_level` | 열 아웃라인 수준 설정 (0-7)                 |
| `write_row`           | 지정한 행 번호에 값 배열 작성                  |
| `write_row_with_style`| 스타일 ID를 적용하여 행 작성                   |
| `add_merge_cell`      | 셀 병합 참조 추가 (예: `"A1:C3"`)            |

#### 성능 참고

StreamWriter는 대규모 쓰기에 최적화되어 있습니다. 각 `write_row()` 호출은 행을 XML로 serialize하여 디스크의 임시 파일(`NamedTempFile`)에 직접 기록합니다. 인라인 문자열(`t="inlineStr"`)을 사용하여 SST 인덱스 재매핑이 필요하지 않으며, 각 행을 독립적으로 serialize할 수 있습니다. 저장 시 임시 파일에서 ZIP 엔트리로 행 데이터를 직접 스트리밍하여, 전체 시트 XML이 메모리에 한꺼번에 존재하지 않습니다.

이 접근 방식은 행 수에 관계없이 O(1) 메모리 사용량을 보장합니다.
...
if (modules) {
  for (const m of modules) {
    console.log(`Module: ${m.name} (type: ${m.moduleType})`);
    console.log(`Source:\n${m.sourceCode}`);
  }
}
